...
AUTHOR_BOOK_API_PORT=$(kubectl get services author-book-api --namespace dev -o go-template='{{(index .spec.ports 0).nodePort}}')
BOOK_REVIEW_API_PORT=$(kubectl get services book-review-api --namespace dev -o go-template='{{(index .spec.ports 0).nodePort}}')
ZIPKIN_PORT=$(kubectl get services my-zipkin --namespace dev -o go-template='{{(index .spec.ports 0).nodePort}}')
